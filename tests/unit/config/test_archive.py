...
from utils.config.run_config import RunConfig, config_to_dict
from utils.core import MODEL_CONFIGS


class TestSaveLoadRoundTrip:
    def test_round_trip_is_field_equal(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
    ...
    def test_concrete_model_class_preserved(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
        rc = make_run_config()
    ...
    def test_none_floats_and_lists_survive(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
        rc = make_run_config()
...
class TestValidation:
    ...
    def test_unknown_field_within_node_raises(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
        import yaml
    ...
    def test_missing_nodes_filled_from_defaults(self, tmp_path: Path) -> None:
        import yaml
    ...
    def test_missing_progress_field_falls_back_to_auto(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
        import yaml
...
class TestMetadata:
    def test_sidecar_written_and_round_trips(
        self, make_run_config: Callable[..., RunConfig], tmp_path: Path
    ) -> None:
